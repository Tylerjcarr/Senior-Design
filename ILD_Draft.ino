...
#define LED_PIN1 1
#define WIDTH 15
#define HEIGHT 20 //height for two matrices stacked vertically. it's actually 15x10
...
const unsigned long idleTime = 150000; //2 1/2 minutes to enter IDLE
const unsigned long animationInterval = 15000; //15 seconds for each animation phase
double lastDistance = 0.0;
const double changeThreshold = 0.5; //threshold in feet, but I need to determine the exact inches I want for LiDAR.
unsigned long measureTime = 0;
const unsigned long timeThreshold = 10;

int position(int x, int y) {
  if (x < 0 || x >= WIDTH || y < 0 || y >= HEIGHT) {
  return -1; //this is to make sure that the animation is within the matrix. this is used for fireworks.
  }
  int i;
  if (y & 1) { //checking if it's an odd row
    i = (y * WIDTH) + (WIDTH - 1 - x); //even rows reversed
  } else {
    i = (y * WIDTH) + x;  //odd rows forward
  }
  return i;
}

void setup() {
  FastLED.addLeds<WS2811, LED_PIN, RGB>(leds, NUM_LEDS / 2); //the first matrix 20tot/2 = 10 which is the actual size.
  FastLED.addLeds<WS2811, LED_PIN1, RGB>(leds + (NUM_LEDS / 2), NUM_LEDS / 2);  //this is the second matrix and this is second matrix where is includes the first with leds + (NUM_LEDS / 2), then accounts for itself with (NUM_LEDS / 2).
...
void loop() {
  ...
  if (lidarSerial.available() >= 9) {
    ...
    if (buf[0] == 0x59 && buf[1] == 0x59) {
      ...
      distance_ft = distance_cm * 0.0328084;  //simple conversion to inches
      Serial.print("Distance: ");
      Serial.println(distance_ft);
    } //else {
      //while (lidarSerial.read() >= 0); //this basically dumps any remaining bytes in the buffer and forcefully reads new data. I was having an issue where the LiDAR would crash.
    //}
    
    unsigned long currentTime = millis();
    if ((fabs(distance_ft - lastDistance) > changeThreshold) && (currentTime - measureTime > timeThreshold)) {
      
      lastDistance = distance_ft;
      sigChange = true;
      currentState = ACTIVE;
      lastChange = millis();
    } else {
  ...
  if (distance_ft <= 3) {
    ...
    beat = 30;
    waveLength = 25;
  } else if (distance_ft <= 4) {
    color = CRGB::Blue;
    beat = 25;
    waveLength = 20;
  } else if (distance_ft <= 5) {
    color = CRGB::Aqua;
    beat = 20;
    waveLength = 17;
...
void sineWave(CRGB color, float beat, float waveLength) {
  ...
  for (int i = 0; i < NUM_LEDS; i++) {
    ...
    float pulse = sin8(timeStamp * 0.3); //play around with
  ...
  //float waveInput = int(timeStamp * beat + x * waveLength) % 255;
  //float y = map(cubicwave8(waveInput), 0, 255, 0, HEIGHT - 1);
  ...
  //leds[position(x, y)] = blend(leds[position(x, y)], color, 30); //play around with. these both control the wave output, so both can't be 0. Although, this controls a wave with the thickness of 1.
    ...
    for (int Y = -1; Y <= 1; Y++) { //adjust for thickness. [0,0] is a single line. [-1, 1] is a line with a thickness of 3. [-2, 2] is a line with a thickness of 5...
      int newY = y + Y;
...
void dots()  {
  ...
  if (timeStamp - lastUpdate >= interval) {
    ...
    if (x == WIDTH || x == -1) { //checks if it needs to bounce
      FastLED.clear();
      direction *= -1; //switching direction
...
void twinkleStar() {
  ...
  if (timeStamp - lastUpdate >= interval) {
    ...
    if (random(0, 3) == 0) { //randomly chooses LEDs to start twinkling. 10) < 2
      int ledIndex = random(NUM_LEDS);
      increasing[ledIndex] = true; // sets an LED as increasing in brightness
      brightness[ledIndex] = 1; //increasing from a low value for a smooth transition
    }

    for (int i = 0; i < NUM_LEDS; ++i) { //increase brightness of LEDs set for twinkling
      if (increasing[i]) {
        if (brightness[i] < 255) {
          brightness[i] += 3; //ncirease for a visible brightening effect
        } else {
...
/*void fireworks() {
  ...
  unsigned long timeStamp = millis();
  static int radius = 0;
  if (timeStamp - lastUpdate > reset && (radius == 8)) {
  ...
  lastUpdate = timeStamp;
  fill_solid(leds, NUM_LEDS, CRGB::Black);
  int centerX = random(WIDTH);
  int centerY = random(HEIGHT);
  CRGB color = CHSV(random8(), 255, 255);
  for (int radius = 0; radius < 8; radius++) {      //maximum radius
    for (int angle = 0; angle < 360; angle += 6) {  //size for denser explosions
...
void raindrops() {
  ...
  for (int x = 0; x < WIDTH; x++) {
      if (random(0, 15) == 0) {
        raindrops[x] = HEIGHT - 1;
      } else {
        if (raindrops[x] >= 0) {
          if (raindrops[x] < HEIGHT) {
            leds[position(x, raindrops[x])] = CRGB::Black;
            raindrops[x] -= 3;
            if (raindrops[x] >= 0) {
...
void spring() {
  ...
  static int column1 = random(WIDTH - 10); //making sure theres room for 3 columns + space for the other rainbow
  static int column2 = (column1 + 6 + random(WIDTH - 16)) % WIDTH; //for the second column making sure some space between and they aren't placed on top of each other
  unsigned long timeStamp = millis();
  if (timeStamp - lastColumn >= columnInterval) {
    lastColumn = timeStamp;
    column1 = random(WIDTH - 10); //new position making sure its room for 3 columns + space
    column2 = (column1 + 6 + random(WIDTH - 16)) % WIDTH; //making sure theres some space between columns
  }
  if (timeStamp - lastUpdate >= interval) {
  lastUpdate = timeStamp; //updates the last update time
  FastLED.clear();
  for (int i = 0; i < 5; i++) { //loop to cover 3 LEDs wide for each column
        for (int y = 0; y < HEIGHT; y++) {
